...
public class Board {
	...
	public void play() {
		...
		while (true) {
			...
			// set our source square with a knight piece, solve move, and print
			// our boards + solutions
	...
	public LinkedList<LinkedList<Square>> getKnightTravailsSolutions(Move move) {
		...
		// return a list of shortest path solution, each solution contains a
		// list of squares it travels
		LinkedList<LinkedList<Square>> solutions = new LinkedList();
		LinkedList<LinkedList<Square>> queue = new LinkedList();// store all the
																// possible
																// paths
		LinkedList<Square> visited = new LinkedList();// store
		boolean solutionFound = false;

		// initialize first path, our very first path is the source square of
		// the move
		// this is technically a path, imagine in a case where our source and
		// destination is the same square
		...
		while (!queue.isEmpty()) {
			...
			// remove the path (1-2-3),
			// but add in further move path later(1-2-3-4,1-2-3-5...)
			...
			if (!solutionFound) {
				for (Square nextLegalSquare : this
						.getKnightNextLegalMoves(currentSquare)) {
	...
	private LinkedList<Square> getKnightNextLegalMoves(Square source) {
		...
		for (Square destination : squares) {
			...
			// since a knight hops over other pieces, we just need to ensure no
			// piece exist within our destination square
			// and, whether we moved 2 then 1 space or 1 then 2 spaces
			if ((destination.getPiece().isBlank())
					&& (Math.abs(xMoveDistance * yMoveDistance) == 2)) {
	...
	private void printWelcome() {
		...
		System.out.println();
		System.out
				.println("Welcome! :) I accept two squares identified by algebraic chess notation.");
		System.out
				.println("The first square is the starting position and the second square is the ending position.");
		System.out
				.println("I will then find the shortest sequence of valid moves to take a Knight piece from the");
		...
		if (solutions.isEmpty()) {
			...
			for (LinkedList<Square> solution : solutions) {
				...
				System.out.println(" SOLUTION #"
						+ ((int) solutions.indexOf(solution) + 1) + ": "
						+ this.getSolutionLine(solution));
			}
		}
	}
	...
	public String getSolutionLine(LinkedList<Square> solution) {
		...
		if (solution.getFirst() == solution.getLast()) {
			...
			for (Square square : solution) {
				if (square != solution.getFirst())
					line += square.toChessNotation() + " ";
			}
		}
		return line;
	}
